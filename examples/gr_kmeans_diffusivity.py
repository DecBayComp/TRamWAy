...
def main():
	...
	# plot
	scalar_map_2d(kmeans, diffusivity)
	plt.title('D (D mode)')
	#plt.show()
	...
	# plot
	scalar_map_2d(kmeans, df['diffusivity'])
	plt.title('D (DF mode)')
	field_map_2d(kmeans, df[[ col for col in df.columns if col.startswith('force') ]])
	plt.title('F (DF mode)')
	#plt.show()
	#sys.exit(0)
	...
	# and plot
	scalar_map_2d(kmeans, dd) # only 'diffusivity' in dd, scalar_map_2d will find alone
	plt.title('D (DD mode)')
	#plt.show()
	...
	# and plot
	scalar_map_2d(kmeans, dv['diffusivity'])
	plt.title('D (DV mode)')
	scalar_map_2d(kmeans, dv['potential'])
	plt.title('V (DV mode)')
