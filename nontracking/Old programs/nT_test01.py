...
setup = {
    'infer': 'nT_test01', 
    'sampling': 'group' ,
    'arguments': {'cell_index': {}, 'dt': {}, 'S': {}, 'p_off': {}, 's2': {}, 'D_bounds': {}, 'D0': {}, 'method': {}, 'tol': {}}
}
...
# Main functions
#-----------------------------------------------------------------------------
def nT_test01(cells, dt=0.04, S=10., p_off=0., mu_on=0., s2=0.05, D_bounds=[0.001,5.], D0=0.2, method='NM',tol=1e-3):
    #index, _, _, _, D0, _, _, _ = smooth_infer_init(cells)
    index = list(cells.keys())
    D=[]
    for i in index:
        D.append(estimate_D_1zone(i,cells,dt,S,p_off,mu_on,s2,D_bounds,D0,method,tol))
    D_out=pd.DataFrame(np.array(D), index=index, columns=['D'])
    print D_out
    return D_out

def estimate_D_1zone(i,cells,dt,S,p_off,mu_on,s2,D_bounds,D0,method,tol):
    assert cells[i]
    cell = cells[i]
    r = cell.r
    t = cell.t
    ...
    print "\nCell number:", i, "N =", len(t), "("+str(np.sum(N_t))+"):", np.average(N_t), "*[", min(t), max(t), "]"
#    print 'N_t =', N_t, 
...
#    print 'dr_ij:', [dr_.shape for dr_ in drs_ij]
    ...
    D_i=fit.x
    print fit, D_i
    D_i_ub=max([1.5*(D_i-s2/dt),np.array([0.])])
