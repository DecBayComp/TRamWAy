...
setup = {
    'infer': 'nT_test01', 
    'sampling': 'group' ,
    'arguments': {'cell_index': {}, 'dt': {}, 'S': {}, 'p_off': {}, 'D_bounds': {}}
}
...
# Main functions
#-----------------------------------------------------------------------------
def nT_test01(cells, cell_index=0, dt=0.04, S=10., p_off=0., mu_on=0., D_bounds=[0.,5.]):
    #index, _, _, _, D0, _, _, _ = smooth_infer_init(cells)
    index = list(cells.keys())
    i = cell_index
    print "Cell number:", i
    D=[estimate_D_1zone(i,cells,dt,S,p_off,mu_on,D_bounds)]
    return pd.DataFrame(np.array(D)) #, index=index, columns=['D'])

def estimate_D_1zone(i,cells,dt,S,p_off,mu_on,D_bounds):
    assert cells[i]
    cell = cells[i]
    r = cell.r
    t = cell.t
    ...
    fittime=time()
    fit = minimize_scalar(marginal_minusLogLikelihood_multiFrame,bounds=D_bounds,\
                                 args=(dt,drs_ij,S,mu_off,mu_on,N_t),method='bounded')
    D_i=fit.x
    print fit
...
#    print("N,M,Delta:",N,M,Delta)
    if(mu_off==0.)&(mu_on==0.)&(Delta==0):
        ...
        mlnL=-log(p_m(0.,0.,mu_on,mu_off))
    elif M==0:
        mlnL=-(log(p_m(0.,Delta,mu_on,mu_off))-lng(N+1)-sum_product_energy(D,dt,dr,gamma,epsilon,max_it,S,N,-Delta,0))
    elif N==0:
        mlnL=-(log(p_m(Delta,Delta,mu_on,mu_off))-lng(M+1)-sum_product_energy(D,dt,dr,gamma,epsilon,max_it,S,N,0,Delta))
    else:
