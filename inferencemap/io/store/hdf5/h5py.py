...
def vlen_poke(service, objname, obj, container):
	dt = special_dtype(vlen=type(obj))
...
def peek_Pandas(service, from_table):
	...
	to_table = File(tmpfilename, 'w', libver='latest')
...
def poke_Pandas(service, objname, obj, to_table):
	...
	from_table = File(tmpfilename, 'r', libver='latest')
...
class HDF5Store(GenericStore):
	...
	def __init__(self, resource, mode='auto'):
		...
		if isinstance(resource, File): # either h5py.File or tables.File
			self.store = resource
		else:
			if mode is 'auto':
				if os.path.isfile(resource):
					self.store = File(resource, 'r', libver='latest')
				else:
					self.store = File(resource, 'w', libver='latest')
			else:	self.store = File(resource, mode)
	...
	def peekNative(self, record):
		try:
			...
			raise AttributeError('hdf5.peekNative', *e.args, record.name)
