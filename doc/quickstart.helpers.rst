...
The first step consists of tessellating the space so that the point data can be partitioned in cells with an adequate number of points and spatial extent.

:mod:`tramway.helper.tessellation` exposes the :func:`~tramway.helper.tessellation.tessellate` method that be called the following way::

	path_to_tracking_data = 'example.trxyt'
	cells = tessellate(path_to_tracking_data, 'grid', avg_location_count=40)
...
* ``'kdtree'``: for a kd-tree tessellation.
* ``'kmeans'``: for a tessellation based on the k-means clustering algorithm.
* ``'gwr'``: for a tessellation based on the Growing-When-Required self-organizing gas.
...
``'kmeans'`` and ``'gwr'`` methods run optimization algorithms on the data and consequently are vulnerable to numerical errors. It is recommended to scale your data::
	...
	tessellate(path_to_tracking_data, method='kmeans', scaling=True)

The tessellation step takes several arguments supposed to control the number of points per cell (``min_cell_count``, ``avg_cell_count``, ``max_cell_count``). This count information is usually taken as hints, and the resulting partition may exhibit cells with a smaller number of points than indicated.
...
To control the cell size at the partition step, a useful option ``knn`` (read "`k` nearest neighbors") ensures an actual minimum number of points per cell and enables cell overlap if necessary::
	...
	tessellate(path_to_tracking_data, method='gwr', scaling=True, knn=40)
...
``'grid'`` and ``'kdtree'`` methods usually have several totally empty cells and it may not make sense to extend such cells until they contain the required number of points. The ``inclusive_min_cell_size`` argument determines how many points a cell should contain to be then extended if necessary. If there are too few points, the cell is ignored and not any point will be associated to this cell::
	...
	tessellate(path_to_tracking_data, method='grid', knn=40, inclusive_min_cell_size=1)
...
In addition, ``'kdtree'`` requires an extra argument so that the cells are not viewed as square or cubic areas. Indeed, square cells have such a variable size that a point in a cell may be closer to the center of another cell. This peculiarity is not compatible with the concept of nearest neighbor::
	...
	tessellate(path_to_tracking_data, method='kdtree', knn=40, inclusive_min_cell_size=1, metric='euclidean')
...
A maximum number of nearest neighbors can also be defined by providing a pair of integers (or ``None``) instead of a single integer::
	...
	tessellate(path_to_tracking_data, method='gwr', knn=(None, 40))
...
A simple command from the :mod:`tramway.helper.tessellation` module::
...
Here ``cells`` can equally be a path to a |h5| tessellation file or the object returned by :func:`~tramway.helper.tessellation.tessellate`.
