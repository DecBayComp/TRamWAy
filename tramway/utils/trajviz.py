...
def trajviz_utility():
    ...
    parser = argparse.ArgumentParser(
        prog="trajviz", description="visualize animated 2D trajectories"
    )
    parser.add_argument(
        "--columns", help="xyt file column names (comma-separated list)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--line-width", type=float, default=1, help="translocation line width"
    )
    parser.add_argument(
        "--marker-style",
        type=str,
        default="o",
        help="location marker style (can be 'none')",
    )
    parser.add_argument(
        "--marker-size", type=int, default=4, help="location marker size"
    )
    parser.add_argument(
        "--axis-off", "--axes-off", action="store_true", help="turn the axes off"
    )
