                ...
                if armijo_rule:
                    ...
                    if gx is not None:
                        ...
                        if c3 is not None:
                            curvature_condition = (
                                c3 * _slope <= new_slope
                            )  # new_slope less negative than slope (can be positive!)
                        else:
            ...
            if eta == eta_max:
                ...
                if a == 0:
                    ...
                    if discr < 0:
                        ...
                        eta1 = (np.sqrt(discr) - b) / (3.0 * a)
                    else:
                        eta1 = -slope / (np.sqrt(discr) + b)
            eta_prev, f_prev = eta, fx
            eta = max(
                c4 * eta, min(eta1, c0 * eta)
            )  # c3 * eta{k} <= eta{k+1} <= c0 * eta{k}
        i += 1
...
def define_pprint():
    ...
    def msg2(_i, _c, *_args):
        ...
        if (
            len(_args) == 3
            and isinstance(_args[1], str)
            and isinstance(_args[-1], (tuple, list))
        ):
...
def extend_global(
    __global__, independent_components, memory, newton, gradient_covariate
):
    ...
    def block(self):
        ...
        if self.fresh:
            if block is None:
                ...
                block = block + sparse.identity(
                    self.gradient_subspace_size, format=block.getformat()
                )
            else:
                block[np.diag_indices(block.shape[0])] += 1.0
            block *= self.eps
        return block
    ...
    def dot(self, g):
        if self.block:
            ...
            for u in self.block:  # for each past block from k-1 to k-m
                if (
                    self.gradient_subspace is None
                    or len(u.s) == self.gradient_subspace_size
                ):
...
class Component(LocalSubspaceProxy, parallel.UpdateVehicle):
    ...
    def __f__(self, _x):
        ...
        return self.__global__.sum(
            [self.__global__.fun(j, _x, *self.__global__.args) for j in self.covariate]
        )
    ...
    def __g__(self, _x, subspace=None, covariate=None, update=False):
        ...
        _total_g, _partial_g = sparse_grad(
            self.__global__.fun,
            _x,
            covariate,
            subspace,
            self.__global__.args,
            self.__global__.sum,
            self.__global__.regul,
            self.__global__.bounds,
            self.__global__.h0,
        )
        return _total_g
    ...
    def commit(self, s):
        ...
        if c.descent_subspace is None:
            if len(s) != self.n:
                raise ValueError(
                    self._format(
                        "wrong size for parameter update `s`; `s` has size {}; full space is {}-dimensional",
                        len(s),
                        self.n,
                    )
                )
            ...
            if len(s) != c.descent_subspace_size:
                raise ValueError(
                    self._format(
                        "parameter update `s` is not in descent subspace; `s` has size {}; descent subspace has size {}",
                        len(s),
                        c.descent_subspace_size,
                    )
                )
...
def minimize_sparse_bfgs1(
    ...
    **kwargs
    ...
    # initial checks
    component, gradient_subspace, descent_subspace, bounds, gradient_sum = _fun_args(
        fun,
        x0,
        component,
        covariate,
    ...
    sched = SBFGSScheduler(
        ...
        ftol=ftol,
        gtol=gtol,
        low_df_rate=low_df_rate,
        low_dg_rate=low_dg_rate,
        newton=newton,
        step_scale=step_scale,
        regul_decay=regul_decay,
        ls_iter_max=ls_iter_max,
        ls_armijo_max=ls_armijo_max,
        ls_wolfe=ls_wolfe,
        ls_regul=ls_regul,
        ls_step_max=ls_step_max,
        ls_step_max_decay=ls_step_max_decay,
        ls_failure_rate=ls_failure_rate,
        fix_ls=fix_ls,
        fix_ls_trigger=fix_ls_trigger,
        verbose=verbose,
        logger=logger,
        diagnosis=diagnosis,
        returns={"f", "df", "projg", "err", "ncalls", "diagnosis"}
        if returns == "all"
        else returns,
    ...
    if verbose:
        ...
        minute = floor(cumt / 60.0)
        second = cumt - minute * 60.0
        if minute:
...
class SBFGSScheduler(parallel.Scheduler):
    ...
    def stop(self, k, i, status):
            ...
            if False:  # status.get('ls_failure', None):
                ...
                if self.fix_ls:
                    ...
                    if self.fix_ls_trigger <= _count:
                        if verbose:
                            self.logger.debug(
                                msg2(k + 1, i, "TRYING TO FIX THE RECURRENT FAILURE")
                            )
                        c.push(x)
                        self.fix_ls(i, x)
                        c.pull(x)
        ...
        try:
            ...
            if (
                self.ftol is not None and ncomponents <= k
            ):  # first epoch ignores this criterion
                if df < self.ftol:
                    self.low_df.add(i)
                    if 0 < self.low_df_rate and df <= 0.1 * self.ftol:
        ...
        try:
            ...
            if (
                self.gtol is not None and ncomponents <= k
            ):  # first epoch ignores this criterion
                if proj < self.gtol:
                    self.low_dg.add(i)
                    if proj <= 0.1 * self.gtol:
...
class SBFGSWorker(parallel.Worker):
    def target(
        ...
        # linesearch
        ls_kwargs = _ls_args(
            step_scale, ls_step_max, ls_iter_max, ls_armijo_max, ls_wolfe, newton
        )
        # linesearch
        if ls_regul:
            ls_regul0 = ls_regul
        if ls_step_max_decay:
            try:
                ...
                final_ls_step_max = initial_ls_step_max * 0.1
        ...
        try:
            while True:
                ...
                try:
                    ...
                    if regul_decay:
                        _decay = max(1.0 - float(k) * regul_decay, 1e-10)
                        if regul:
                            __global__.regul = regul = regul0 * _decay
                        if ls_regul:
                            ls_regul = ls_regul0 * _decay
                    if ls_step_max_decay:
                        ls_step_max = initial_ls_step_max * max(
                            1.0 - float(k) * ls_step_max_decay, final_ls_step_max
                        )
                    ...
                    if s is None:
                        if verbose:
                            ...
                            logger.info(
                                msg2(k + 1, i, "LINE SEARCH FAILED ({})".format(res))
                            )  # .upper())))
                    ...
                    if h is None:
                        if verbose:
                            logger.debug(
                                msg2(k + 1, i, "GRADIENT CALCULATION FAILED (k+1)")
                            )
        ...
        try:
            ...
            if ncomponents <= k:  # first epoch ignores the following criterion
                if s is None:
                    ...
                    # try fixing problematic components
                    if fix_ls:
                        _count = recurrent_ls_failure_count[i] = (
                            recurrent_ls_failure_count.get(i, 0) + 1
                        )
                        if fix_ls_trigger <= _count:
                            if verbose:
                                logger.debug(
                                    msg2(k, i, "TRYING TO FIX THE RECURRENT FAILURE")
                                )
                            c.push(x)
                            fix_ls(i, x)
                            c.pull(x)
            ...
            # check for convergence based on f
            if (
                ftol is not None and ncomponents <= k
            ):  # first epoch ignores this criterion
                df = c.f - c1.f
            ...
            # check for convergence based on g
            if (
                gtol is not None and ncomponents <= k
            ):  # first epoch ignores this criterion
...
def sparse_grad(
    fun, x, active_i, active_j, args=(), _sum=np.sum, regul=None, bounds=None, h0=1e-8
):
    """
        ...
        try:
            for u, h in enumerate(H):
                try:
                    ...
                    #
                    partial_grad_j = (f_a - f_b) / (2.0 * h)
                    if regul:
                        penalty = regul * 2.0 * xj
                ...
                for v in range(1, a.shape[1]):
                    ...
                    a[u, v] = a_uv = (a_up * fac - a_pp) / (fac - 1.0)
                    fac *= CON2
    ...
    if any_ok:
        if failures:
            module_logger.warning(
                "sparse_grad failed at column(s): {}".format(
...
__all__ = [
    ...
    "minimize_sparse_bfgs0",
    "minimize_sparse_bfgs1",
    "SparseFunction",
    "wolfe_line_search",
    "sparse_grad",
]
