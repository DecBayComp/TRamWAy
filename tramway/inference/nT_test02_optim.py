...
# Main functions
#-----------------------------------------------------------------------------
def nT_test02_optim(cells, dt=0.04, S=10., p_off=0., mu_on=0., s2=0.0025, D0=np.array([0.2,0.2]), method='NM',tol=1e-3,times=[0.]):
    #index, _, _, _, D0, _, _, _ = smooth_infer_init(cells)
    index = list(cells.keys())
    D=[]
    for i in index:
        D.append(estimate_D_2zones(i,cells,dt,S,p_off,mu_on,s2,D0,method,tol,times))
...
def estimate_D_2zones(i,cells,dt,S,p_off,mu_on,s2,D0,method,tol,times):
    ...
    print "\nCell no.:", i, "; sum(N):", np.sum(N_in), ", possible links:", poss_links_in
