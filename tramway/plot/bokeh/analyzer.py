    ...
    def select_mapping(self, mapping_label):
        ...
        if self.has_time_segments():
            ...
            self.current_mapping = {
                feature: t.split_segments(m[feature]) for feature in m.features
            }
...
class Controller(object):
    ...
    def __init__(self, model, side_panel=None, webdriver=None, **map_kwargs):
        ...
        self.source_dropdown = Select(
            title="Data source:", options=["None"] + options if options[1:] else options
        )
        ...
        self.source_dropdown.on_change(
            "value", lambda attr, old, new: self.load_source(new)
        )
        self.sampling_dropdown.on_change(
            "value", lambda attr, old, new: self.load_sampling(new)
        )
        self.mapping_dropdown.on_change(
            "value", lambda attr, old, new: self.load_mapping(new)
        )
        self.feature_dropdown.on_change(
            "value", lambda attr, old, new: self.load_feature(new)
        )
        self.trajectories_kwargs = dict(
            color="r", line_width=0.5, line_alpha=0.5, loc_alpha=0.1, loc_size=6
        )
        self.show_side_panel = (
            webdriver is not None if side_panel is None else side_panel
        )
    ...
    def load_source(self, source_name):
        ...
        try:
            ...
            self.source_dropdown.options = [
                src for src in self.source_dropdown.options if src != source_name
            ]
            if not (
                self.source_dropdown.options
                and (
                    self.source_dropdown.options[1:]
                    or self.source_dropdown.options[0] != "None"
                )
            ):
    ...
    def make_time_view(self):
        ...
        self.time_slider.on_change(
            "value_throttled", lambda attr, old, new: self.refresh_map()
        )
        return self.time_slider

    def make_space_view(self):
        self.main_figure = f = figure(
            disabled=True,
        ...
        # f.visible = False
        self.visibility_controls = CheckboxGroup(
            disabled=True,
            labels=["Localizations", "Trajectories", "Hide map"],
            active=[],
        )
    ...
    def draw_map(self, feature):
        ...
        if self.main_figure.renderers:
            self.main_figure.renderers = []
        try:
            self.visibility_controls.active = [
                pos for pos in self.visibility_controls.active if pos != 2
            ]
        except ValueError:
            pass
        ...
        if _map.shape[1] == 2:
            _vector_map = _map
            if feature == "force":
                _map = _map.pow(2).sum(1).apply(np.log) * 0.5
            else:
                _map = _map.pow(2).sum(1).apply(np.sqrt)
        self.map_glyphs = scalar_map_2d(
            _cells,
            _map,
            figure=self.main_figure,
            colorbar_figure=self.colorbar_figure,
            **kwargs
        )
        if _vector_map is not None:
            self.map_glyphs.append(
                field_map_2d(
                    _cells, _vector_map, figure=self.main_figure, inferencemap=True
                )
            )
        elif _map.shape[1] != 1:
    ...
    def draw_trajectories(self):
        ...
        traj_handles = plot_trajectories(
            sampling.points, figure=self.main_figure, **self.trajectories_kwargs
        )
    ...
    def enable_side_panel(self):
        ...
        self.export_file_input.value = "_".join(
            [
                part
                for part in (
                    self.source_dropdown.value,
                    self.sampling_dropdown.value,
    ...
    def make_side_panel(self):
        self.export_file_input = TextInput(
            disabled=True, value="", title="append any of: .png, .svg, .txt, .csv"
        )
        self.figure_export_button = Button(
            disabled=True, label="Export figure", button_type="success"
        )
        self.data_export_button = Button(
            disabled=True, label="Export data", button_type="success"
        )
        ...
        layout = column(
            self.export_file_input, self.figure_export_button, self.data_export_button
        )
        if not self.show_side_panel:
    ...
    def set_export_status(self, what, status):
        if what == "figure":
            ...
            raise ValueError(
                "'{}' not supported; accepted values are: 'figure', 'data'".format(what)
            )
        ...
        try:
            ...
            if output_file.endswith(".png"):
                ...
                raise NotImplementedError(
                    "format '{}' not supported".format(os.path.splitext(output_file)[1])
                )
    ...
    def export_data(self, output_file):
        ...
        try:
            ...
            index = list(df.index)
            df = df.join(
                pd.DataFrame(xy[index], index=index, columns=["center x", "center y"])
            )
...
def browse_maps(analyzer, **kwargs):
    ...
    if not sys.argv[0].endswith("bokeh"):
        if analyzer.env.script and not (
            os.path.isabs(analyzer.env.script)
            and sys.argv[0] == os.path.basename(analyzer.env.script)
        ):
            print("running bokeh server...\n")
            import subprocess

            p = subprocess.Popen(
                [sys.executable, "-m", "bokeh", "serve", "--show", analyzer.env.script],
                cwd=os.getcwd(),
            ...
            if not hasattr(main, "__file__"):
                print(
                    """bokeh map browser cannot run in an interpreter
