        ...
        if min_points:
            if min_points != start_stop_min_points:
                ...
                if not np.all(ok):
                    ...
                    assert np.all(0 < gap)
                    max_gap = window - shift
                    gap_ok = gap <= max_gap
                    if not np.all(gap_ok):
                        ok_first = np.r_[first_t,  ok_first[~gap_ok]]
                        ok_last = np.r_[ok_last[~gap_ok], last_t]
                        # TODO: consider all segments instead of the longest
                        segment_dur = times[ok_last] - times[ok_first] # + window
                        assert np.all(0 <= segment_dur)
                        s = np.argmax(segment_dur)
                        first_t = ok_first[s]
                        last_t = ok_last[s]
