...
class Browser(AnalyzerNode):
    ...
    def show_maps(self, **kwargs):
        ...
        if (
            self.this_notebook_cell_only
            and self.script is not None
            and self.script.endswith(".ipynb")
        ):
            self.logger.info(
                """\
...
"""
            ...
            if isinstance(self.this_notebook_cell_only, bool):
                ...
                if not caller_cells:
                    raise RuntimeError(
                        "cannot find an expression calling `browser.show_maps`"
                    )
                if caller_cells[1:]:
                    raise RuntimeError(
                        "multiple `browser.show_maps` calls found; please specify the index of the notebook cell"
                    )
                ...
                try:
                    ...
                    self.logger.debug(
                        f"notebook cell index {notebook_cell_index}:\n\n"
                        + "".join(caller_cell)
                    )
            condensed_code = []
            for line in caller_cell:
                ...
                if (
                    ...
                    and "this_notebook_cell_only" not in line
                ):
            ...
            try:
                ...
                p = subprocess.Popen(
                    [sys.executable, "-m", "bokeh", "serve", "--show", tmpfile],
                    cwd=os.getcwd(),
